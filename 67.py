import pandas as pd
#“2020.10.18 RVTools数据处理 v5.5-80.7.176.14”
#读写路径
fill_path = './data_input/RVTools_export_all_76.7.34.19(60).xlsx'
out_path = './data_output/76.7.34.19(60)/'
...
#集群主机内存及CPU的大小、型号检查
CPU_df = pd.read_excel(fill_path, sheet_name='vHost', usecols='A:C,G,O')
...
#主机CPU负载健康状态
CPU_usage = pd.read_excel(fill_path, sheet_name='vHost', usecols='A:C,N')
CPU_usage = CPU_usage[CPU_usage['CPU usage %'] > 80]
CPU_usage.to_excel(out_path + '主机CPU负载健康状态-详细信息.xlsx', index=None)


#主机内存负载健康检查
Memory_usage = pd.read_excel(fill_path, sheet_name='vHost', usecols='A:C,P')
...
tools_df = pd.read_excel(fill_path, sheet_name='vTools', usecols='A:B,E')
...
vDisk_df = pd.read_excel(fill_path, sheet_name='vDisk', usecols='A,B,U')
